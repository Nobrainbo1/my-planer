...
| New project needs a starter | "Plan a small mobile app; I have no existing code or preferred stack." | Uses the discovery map to find a relevant official starter, verifies its current requirements, and compares it with building without a starter. Does not treat the map as a fixed install list or recommend an unrelated agent runtime. |
